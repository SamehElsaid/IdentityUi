      ...
      renderCell: params => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    onClick: () => {
                      ...
                      axiosGet(
                        ...
                        .then(res => {
                          if (res.status) {
                            toast.success(
                              !params.row.isActive ? messages.activeUserSuccess : messages.inactiveUserSuccess
                            )
                            setData(
                              data.map(item =>
          ...
          <Tooltip title={messages.block}>
            <IconButton
              ...
              onClick={() => {
                ...
                if (blockOpen !== params.row.id) {
                  toast.info(messages.userPage.areYouSureBlock, {
                    position: locale === 'ar' ? 'bottom-left' : 'bottom-right',
                    autoClose: 4000,
                    hideProgressBar: false,
                    closeOnClick: true,
                    ...
                    iconPosition: 'left',
                    onClick: () => {
                      const loadingToast = toast.loading(messages.block + '...')
                      axiosGet(`User/BanUser/?userId=${params.row.id}`, 'en')
                        .then(res => {
                          if (res.status) {
                            toast.success(messages.userPage.blockSuccess)
            ...
            >
              <IconifyIcon icon='solar:user-block-outline' />
            </IconButton>
          </Tooltip>
        </Box>
      )
